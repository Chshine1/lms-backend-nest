...
| Field              | `emailVerifiedAt`   | `TIMESTAMP`     | Timestamp when email was verified. `null` if not yet verified. (PRD Mapping: Registration email verification)                                                                                                                                                                                 | Nullable, set by `markEmailVerified()` method.                         |
...
| **Entity Method**  | `markEmailVerified` | -               | Records the timestamp when email verification is confirmed. Sets `emailVerifiedAt` to current timestamp. (PRD Mapping: Registration email verification)                                                                                                                                       | Sets `emailVerifiedAt` to current date/time.                           |
...
| **Entity Method**  | `completeOnboarding` | -               | Transitions `onboardingStatus` to `COMPLETED`. (PRD Mapping: OC Onboarding completion step)                | Throws `OnboardingAlreadyCompletedException`. Publishes `StudentOnboardingCompleted`. |
...
| Field       | `assignedBy`     | `BIGINT`        | References the `User.id` of the administrator who made the assignment. (PRD Mapping: 1.3 Admin assigns roles)                  | `FOREIGN KEY (assignedBy) REFERENCES User(id)`, `NOT NULL`. All role assignments must be tracked with an admin user. |
...
| **UserApplicationService**       | `findById`                 | `userId`                                              | `UserDto \| null`    | Fetches user by ID from repository and maps to DTO. (PRD Mapping: User retrieval)                                                                                                  |
| **UserApplicationService**       | `authenticate`             | `username`, `password`                                | `string` (JWT token) | Finds user by email (username), compares password hash, returns access token. Used for login flows. (PRD Mapping: Authentication/Login)                                            |
...
| :------------------------------ | :---------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------- |
| **RegistrationService**         | `registerUser`    | Validates invitation code (via `Tenant.isInvitationValid()`), checks email/phone uniqueness, hashes the plain password, and creates a `User` aggregate. Returns the new `User`. Tenant ID is derived from the invitation code. (PRD Mapping: Sign-up Information - Invitation code validation, email/phone uniqueness, password hashing) | `TenantRepository`, `UserRepository`, `PasswordHasher`.            |
